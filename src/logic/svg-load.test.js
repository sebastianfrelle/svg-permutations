...
let hasFillSelector = '*[fill]:not([fill="none"])';
let hasStrokeSelector = '*[stroke]:not([stroke="none"])';

test('parsing svg', () => {
  let parser = new DOMParser();
  let svgDoc = parser.parseFromString(svgWithStrokeCode, 'image/svg+xml');
  
  let hasFillCount = svgDoc.querySelectorAll(hasFillSelector).length;
  let hasStrokeCount = svgDoc.querySelectorAll(hasStrokeSelector).length;

  // Ex: 4 non-null, non-none `fill`s and 2 non-null, non-none `stroke`s
  // requires 6 colors in total.
  let permutationLength = hasFillCount + hasStrokeCount;
  ...
  for (const cp of colorPermutations) {
    ...
    let colorIndex = 0;
    copy.querySelectorAll(hasFillSelector).forEach((node) => {
      node.setAttribute('fill', cp[colorIndex++]);
    });

    copy.querySelectorAll(hasStrokeSelector).forEach((node) => {
